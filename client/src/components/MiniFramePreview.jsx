...
import { FRAME_STYLES, MINI_FRAME_MAT_PADDING } from "../lib/frameStyles.js";

function isUrlBackground(bg) {
  return typeof bg === "string" && bg.trim().startsWith("url(");
}

export default function MiniFramePreview({ imageUrl, frame, mat }) {
  const safeFrame = FRAME_STYLES[frame] ? frame : "Black Wood";
  const frameStyle = FRAME_STYLES[safeFrame];
  const matPadding = MINI_FRAME_MAT_PADDING[mat] ?? "0px";
