...
// Xóa import ShinyText vì không tồn tại → fix build error
...
export default function BlogPostPage() {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        const response = await api.get(`/blogs/${slug}`); // Nếu route khác, chỉnh ở đây (ví dụ /api/posts/${slug})
        setBlog(response.data); // Giả sử response.data là object blog trực tiếp; nếu {blog: {...}} thì setBlog(response.data.blog)
        setLoading(false);
      } catch (err) {
        console.error("Lỗi tải bài viết:", err);
        setError("Không tìm thấy bài viết hoặc lỗi kết nối. Vui lòng thử lại.");
        setLoading(false);
      }
    };
  ...
  if (error || !blog) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center flex-col gap-6">
        <p className="text-2xl font-bold text-red-600">{error || "Bài viết không tồn tại"}</p>
        <Link
          to="/blog"
          className="px-6 py-3 bg-black text-white font-bold uppercase tracking-wider hover:bg-gray-800 transition"
        >
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] font-sans antialiased selection:bg-[#00ffff]/30">
      ...
      <section className="relative h-[60vh] md:h-[80vh] overflow-hidden bg-slate-900">
        <motion.img
          style={{ y: yImage }}
          src={blog?.coverImage?.url || "https://via.placeholder.com/1920x1080?text=Cover+Image"}
      ...
      <main className="mx-auto max-w-7xl px-6 py-32 grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <article className="lg:col-span-8 lg:col-start-3">
          <div
            ...
            dangerouslySetInnerHTML={{ __html: blog?.content || "<p>Nội dung bài viết đang được cập nhật...</p>" }}
          />
